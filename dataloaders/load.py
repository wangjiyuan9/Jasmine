...
class Hypersim(Dataset):
    def __init__(self, root_dir, transform=True, near_plane=1e-5, far_plane=65.0):
        ...
        self.split_path = os.path.join(root_dir, "filename_meta_train.csv")
        ...
        for _, row in df.iterrows():
            if row['included_in_public_release'] and (row['split_partition_name'] == "train"):
                ...
                if os.path.exists(rgb_path) and os.path.exists(depth_path):
                    pair_info = {'rgb_path': rgb_path, 'depth_path': depth_path}
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            rgb_tensor, depth_tensor = self.transform(rgb_image, depth_image)
        else:
        ...
        return {"rgb": rgb_tensor, "depth": depth_tensor, 'metric': metric_tensor, "val_mask": valid_depth_mask, "domain": "indoor"}
...
class VirtualKITTI2(Dataset):
    ...
    def _find_pairs(self):
        ...
        vkitti2_rgb_path = os.path.join(self.root_dir, "rgb")
        vkitti2_depth_path = os.path.join(self.root_dir, "depth")
    ...
    def __getitem__(self, idx):
        ...
        return {"rgb": rgb_tensor, "depth": depth_tensor, 'metric': metric_tensor, "val_mask": valid_depth_mask, "domain": "outdoor"}
