...
def is_internet_connected(url='http://www.baidu.com', timeout=5):
    try:
        ...
        if is_internet_connected():
            try:
                ...
                if num_layers == 18:
                    ...
                    loaded = torch.hub.load_state_dict_from_url(weights.url, check_hash=False)
                elif num_layers == 50:
                    weights = models.ResNet50_Weights.IMAGENET1K_V1
                    loaded = torch.hub.load_state_dict_from_url(weights.url, check_hash=False)
            except Exception as e:
                print(f"下载预训练模型失败: {e}")
                print("尝试使用本地模型或不使用预训练权重...")
                return model
        else:
            try:
                loaded = torch.load(args.pretrain_presnet_path)
            except:
                print("未连接网络且未找到本地预训练模型，使用随机初始化...")
                return model
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1, inputs_dim=3):
        ...
        if num_input_images > 1:
            ...
            if inputs_dim != 3:
                self.encoder.conv1 = nn.Conv2d(inputs_dim, 64, kernel_size=7, stride=2, padding=3, bias=False)
